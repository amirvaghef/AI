Public Class BezierSpline : Inherits Polygon
   ...
   Protected Overrides Function InterpolateSegment( _
      ...
      Dim Pts As PointF() = Me.DrawPath.PathPoints
      Dim BezierSegment As PointF() = New PointF() { _
         Pts(Indx - 3), Pts(Indx - 2), Pts(Indx - 1), Pts(Indx)}
      'binary aproximation
      Dim Range As Single() = New Single() {0, 0, 1}  'elsewhere known as: Bottom, Mid, Top
      Dim Dlt As Single = -2.0F
      Dim Pt As PointF
      Do
         If Dlt < 0 Then  'set Bottom or Top to Mid
            Range(0) = Range(1)
         Else
            Range(2) = Range(1)
         End If     
   ...
   Private Shared Function PointOfFraction( _
      ...
      For I As Integer = 0 To ptBeziers.Length - 2 'first walk-through fills a temporary array
         Pts(I) = PointBetween(ptBeziers(I), ptBeziers(I + 1), Fraction)
      Next
      For UBord As Integer = Pts.Length - 2 To 0 Step -1 'following loops overwrite the array
         For I As Integer = 0 To UBord
   ...
   Private Shared Function PointBetween( _
      ...
      PointBetween = DrawingX.Mult(Pt2 - New SizeF(Pt1), Fraction)
      PointBetween += New SizeF(Pt1)
   End Function
