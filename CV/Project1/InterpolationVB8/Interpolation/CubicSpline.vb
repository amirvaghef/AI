Public Class CubicSpline : Inherits Polygon
   ...
   Private Shared Sub SolveTridiag( _
      ...
      Dim N As Integer = Results.Length - 1
      Dim Helper1 As Single = Diag(0)
      For I As Integer = 1 To N
         Dim Helper2 As Single = SubDiag(I) / Helper1
   ...
   Protected Overrides Sub PreparePath()
      Dim NP As Integer = _Points.Length
      ...
      For I As Integer = 0 To NP - 3
         ...
         Dim DeltaXBefore As Single = Pt.X - PtBefore.X
         Dim DeltaXAfter As Single = PtAfter.X - Pt.X
         If DeltaXAfter = 0 Then Return 'a vertical spline-segment is not interpolable

         Dim GradeBefore As Single = (Pt.Y - PtBefore.Y) / DeltaXBefore
         Dim GradeAfter As Single = (PtAfter.Y - Pt.Y) / DeltaXAfter
      ...
      For Each Pt As PointF In PathPoints
   ...
   Protected Overrides Function InterpolateSegment(ByVal X As Single, ByVal Indx As Integer) As PointF
      ...
      Dim DeltaX As Single = PtTo.X - PtFrom.X
      Dim x1 As Single = X - PtFrom.X
      Dim x2 As Single = DeltaX - x1
      'some numerical black magic again...
      Dim Y As Single = ((_Results(Indx - 1) * x1 * (x2 + DeltaX) + PtFrom.Y) * x2 + _
